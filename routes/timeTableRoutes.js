...
const verifyCourseTeacherPair = async (courseId, teacherId) => {
  ...
  if (!assigned) {
    return {
      ok: false,
      message: "Selected teacher is not assigned to this course",
    };
  }
...
router.post("/addTimeTableEntry", authMiddleWare, async (req, res) => {
  try {
    ...
    const conflict = await TimeTable.findOne(
      buildConflictQuery({ course, teacher, dayOfWeek, startTime, endTime }),
    );
      ...
      return res.status(409).json({
        ...
        message:
          String(conflict.teacher) === teacher
            ...
            : "Course already has a timetable entry in this time slot",
      });
    }
